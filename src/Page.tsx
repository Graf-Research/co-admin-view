...
export function Page<T extends DataType, U extends FieldDataType>(props: PageProps<T, U>) {
  ...
  async function getInitialOptionData() {
    for (const column of props.columns.filter((tc: TableColumn) => tc.kind === 'option-filter')) {
      if ((column as TableColumnOptionFilter).options) {
        list_options[column.key] = (column as TableColumnOptionFilter).options!;
      }
      if ((column as TableColumnOptionFilter).getOptionData) {
        list_options[column.key] = await (column as TableColumnOptionFilter).getOptionData!();
      }
    }
